  ...
  */
  createWindow() {
    ...
    */
    ...
    loginButton.href =
      "https://discord.com/api/oauth2/authorize?client_id=1025037341724774431&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fauth%2Fdiscord&response_type=token&scope=identify%20guilds%20guilds.members.read";
    loginButton.innerText = "Login";
    loginButton.classList.add("my-button");
    windowDiv.appendChild(loginButton);
...
const createStylesheet = () => {
  ...
  style.innerHTML = `
    .my-button {
      ...
      background: linear-gradient(rgb(0 162 204) 17.14%, rgb(8 111 168) 76.68%);
      box-shadow: rgb(0 0 0 / 34%) 0px -3px 0px inset;
    }
...
setTimeout(() => {
  ...
  */
  ...
  hackState.loginEvent = new Event("__login");
  ...
  */
  ...
  console.log("token_type:", tokenType, "access_token:", accessToken);
  if (accessToken) {
    console.log("access_token available");

    hackState.accessToken = accessToken;
    hackState.tokenType = tokenType;

    console.log("fetching guild member...");
    try {
      fetch(
        "https://discord.com/api/users/@me/guilds/728017027184328785/member",
        {
          headers: {
        ...
        .then((data) => {
          ...
          hackState.username = username;
          hackState.roles = roles;

          eventElement.dispatchEvent(hackState.loginEvent);
        })
        .catch((error) => {
